...
figma.ui.onmessage = (message) => {
    ...
    if (message.type === 'import-screenshots') {
        try {
            ...
            const messageText = error instanceof Error ? error.message : '导入截图时发生未知错误。';
            figma.notify(`导入失败：${messageText}`, { error: true });
        }
    }
};
function importScreenshots(items) {
    if (items.length === 0) {
...
function createCaptureFrame(item) {
...
function createImageNode(itemName, segment, index) {
...
function createRootName(sourceUrl) {
